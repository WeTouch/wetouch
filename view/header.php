      ...
      <?php if(isset($_SESSION["id"])){ ?>
        <div class="collapse navbar-collapse">
          <ul class="nav navbar-nav navbar-right">
            ...
            <li><a href="<?php echo $app->urlFor('chat');?>">Chat</a>
              <?php
              if (isset($_SESSION['notification']))
              {
                $count = 0;
                foreach($this->data['lastTime'] as $time)
                {
                  $timestamp =  strtotime($time);
                  $lastMessageSec = time() - $timestamp;
                  $lastViewSec = time() - $_SESSION['notification'];
                  if ($lastMessageSec < $lastViewSec)
                  {
                    $count += 1;
                  }
                }
